-- Buy-in model: drop the $50 fee (and any interest). A $500 buy-in now creates
-- exactly $500 of debt and credits the full $500 as cash. Rebuilds
-- request_loan / approve_loan to stop charging the fee and stop writing the
-- loan_fee ledger row.
begin;

-- New buy-in requests carry no fee.
alter table public.loan_requests alter column fee set default 0;
...
begin
  ...
  values (v_season, p_user_id, 500, 0, 'pending') returning id into v_req_id;
...
begin
  ...
  -- No fee: cash and debt both increase by exactly $500.
  v_new_balance := round(v_player.current_balance + 500, 2);
  v_new_debt    := round(v_player.outstanding_debt + 500, 2);
  ...
  -- Single principal row credits the full $500 as cash (no loan_fee row).
