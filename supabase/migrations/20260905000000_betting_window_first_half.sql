-- Open in-play (走場) betting through the end of the first half: allow bets
-- until 45 minutes after kickoff (was 30). Rebuilds place_single_bet /
-- place_parlay with the new cutoff; everything else is unchanged.
begin;
...
begin
  ...
  -- Betting cutoff: closed at kickoff plus 45 minutes (end of first half),
  -- or non-open status.
  if v_match.status in ('FINISHED','CANCELLED','POSTPONED','SUSPENDED','AWARDED')
     or now() >= (v_match.kickoff_time + interval '45 minutes') then
...
begin
  ...
  -- Every leg must be in the active season and before its cutoff (kickoff + 45m).
